...
def build_provider(http_open=None):
    ...
    return SndProvider(
        ...
        user_agent="debenture-search-smoke-test/1.0",
    )


def run_smoke_test(asset_code, provider=None, parser=None):
    """Executa uma unica consulta sem gravar no banco."""

    selected_provider = provider or build_provider()
    selected_parser = parser or SndParser()
    normalized_code = selected_provider.normalize_asset_code(
        asset_code
    )

    response = selected_provider.fetch_html(
        normalized_code
    )
    parsed = selected_parser.parse(response.text)
